...
function createSubjects(scene)
	...
	wordsTwo = createWords("Наверняка тут спрятано множество ловушек.");
	wordsThree = createWords("Предлагаю обойти эту деревню.");
	wordsFour = createWords("Поддерживаю, тем более спешить нам пока не куда.");
	wordsFive = createWords("А как же сокровища?");
	wordsSix = createWords("Мне не терпится поскорее отыскать Эльдорадо!");
	wordsSeven = createWords("И вот, спустя 500 лет, группа отважных кладоискателей нашла затерянные сокровища»");
	wordsEight = createWords("я уже вижу название статьи и себя на первой странице всех печатных СМИ.");
	wordsNine = createWords("Помечтаем позже. Мы уже близки к цели, давайте ускоримся.");
...
function onBackBtn()
	-- composer.setVariable( "checkpoint", 3 );
...
function scene:show( event )
	...
	elseif phase == "did" then
		...
		if bgText.alpha == 0 then
			transition.to( faceNine, { time=800, alpha=1 });
			transition.to( nameOne, { time=800, alpha=1 });
